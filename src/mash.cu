...
(
    ...
    while (uni<sketchSize)
    {
        if (A[APtr]==B[BPtr]) inter+=1, APtr++, BPtr++;
...
(
    ...
    int bs = blockDim.x;
    int idx = tx+bx*bs;
    if(idx>=d_numSequences) return;
    if(idx == rowId){
        d_mashDist[rowId*d_numSequences+idx] = 0;
        return;
    }
    ...
    d_mashDist[rowId*d_numSequences+idx] = mashDist;
}
...
(   
    ...
    for(int i=0;i<d_numSequences;i++){
        int numBlocks=(d_numSequences+255)/256,numThreads=256;
        mashDistConstruction<<<numBlocks,numThreads>>>(i, d_hashList, d_seqLengths, d_numSequences, d_mashDist, params.kmerSize, params.sketchSize);
    }
...
void GpuSketch::DeviceArrays::printMashDist(uint64_t h_numSequences, std::vector<std::string> seqs) 
{
    ...
    double * h_mashDist = new double[h_numSequences*h_numSequences];

    double * mashDist = d_mashDist;

    cudaError_t err;


    err = cudaMemcpy(h_mashDist, mashDist, (h_numSequences*h_numSequences)*sizeof(uint64_t), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) {
        fprintf(stderr, "Gpu_ERROR: cudaMemCpy failed!\n");
        exit(1);
    }
