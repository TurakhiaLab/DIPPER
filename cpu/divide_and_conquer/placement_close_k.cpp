...
)
{
    ...
    int otheid = head[y];
    while (e[otheid] != x)
        assert(e[otheid] != -1), otheid = nxt[otheid];

    int leafCount = 0;
    for (int i = 0; i < 5; i++)
        leafMask[leafCount++] = closest_id[eid * 5 + i];
    for (int i = 0; i < 5; i++)
        leafMask[leafCount++] = closest_id[otheid * 5 + i];
    ...
    const MSADeviceArraysDC &msaDeviceArrays)
{
    ...
    for (int i = numSequences; i < totalNumSequences; i++)
        contains[cluster_id[i]].push_back(i);

    int insertLeafCount = numSequences;

    std::vector<int> containsPrefixSum(numSequences * 4 - 4);
    tbb::parallel_scan(tbb::blocked_range<size_t>(0, contains.size()), 0, [&](const tbb::blocked_range<size_t> &r, int sum, bool is_final_scan) -> int
                       {
            ...
            return sum; }, [](int left, int right) -> int
                       { return left + right; });
