...
void MashPlacement::MashDeviceArrays::allocateDeviceArrays(uint64_t **h_compressedSeqs, uint64_t *h_seqLengths, size_t num, Param &params)
{
    ...
    auto wordsPerSeq = [&](uint64_t len) -> uint64_t {
        return params.isProtein ? (len + 7) / 8 : (len + 31) / 32;
    };
    for (size_t i = 0; i < numSequences; i++)
        flatStringLength += wordsPerSeq(h_seqLengths[i]);
    ...
    // generate prefix array (per-sequence word count)
    for (int i = 0; i < numSequences; i++)
    {
        d_prefixCompressed[i] = params.isProtein ? (d_seqLengths[i] + 7) / 8 : (d_seqLengths[i] + 31) / 32;
    }
...
void MashPlacement::MashDeviceArrays::printSketchValues(int numValues)
{
    ...
    std::memcpy(h_hashList, hashList, numSequences * 1000 * sizeof(uint64_t));
...
static void decompress_protein_kmer_cpu(
...
void sketchConstruction(
    ...
    uint64_t kmerSize,    // in bases / residues
    bool isProtein
)
{
        ...
        while (iteration <= seqLength - kmerSize) {
            for (int j = iteration; j < iteration+threadsPerBlock; ++j) {
                ...
                if (j <= seqLength - kmerSize) {
                    ...
                    uint8_t out[16];
                    if (isProtein) {
                        decompress_protein_kmer_cpu(compressedSeqs, j, kmerSize, kmer_fwd);
    ...
    int numSequences)
{
    ...
    // for (int idx = 0; idx < rowId; ++idx) {
        ...
        d_mashDist[idx] = std::min(1.0, fabs(log(2.0*jaccardEstimate/(1.0+jaccardEstimate))/kmerSize));
    } });
}
