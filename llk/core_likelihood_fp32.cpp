...
void scale(size_t i, std::vector<std::vector<float>> &bottom, utility::SCALE_TYPE SCALE_TYPE_ASSIGN, float max_prob, float sum_prob, double max_exponent, utility::Node *node)
{
    for (size_t j = 0; j < STATES; j++)
    {
        if (SCALE_TYPE_ASSIGN == utility::MAX)
            ...
            bottom[i][j] /= sum_prob;
        else if (SCALE_TYPE_ASSIGN == utility::EXP)
            bottom[i][j] /= max_exponent;
    ...
    if (SCALE_TYPE_ASSIGN == utility::MAX)
        ...
        node->scale_vector[i] *= max_exponent;
}

#include <cmath> // 引入头文件
#include <fstream>

std::ofstream multiplication_values_file;
        ...
        for (size_t i = 0; i < bottom.size(); i++)
        {
            ...
            for (size_t j = 0; j < STATES; j++)
            {
                ...
                for (size_t k = 0; k < STATES; k++)
                {
                    ...
                    float product = child_bottom[i][k] * mat_out[j][k];
                    multiplication_values_file << product << "\n";
                    prob += product;
                }
                bottom[i][j] *= prob;
                sum_prob += bottom[i][j];

                frexpf(bottom[i][j], &cur_exp);

                if (cur_exp > max_exp)
                {
                    max_exp = cur_exp;
                    // scale_exp = pow(2,-max_exp);
                }

                if (max_prob < bottom[i][j])
                {
                    max_prob = bottom[i][j];
                }
            }
            ...
            // printf("scale_exp: %f\n", scale_exp);
            scale_exp = pow(2, max_exp);
            scale(i, bottom, utility::SCALE_TYPE_ASSIGN, max_prob, sum_prob, scale_exp, node);
        }
    }
...
double postorder_traversal_scaling(utility::Node *node)
{
    double lk = 0.0;
    for (auto &child : node->children)
        lk += postorder_traversal_scaling(child);
...
void top_down_preorder_traversal(utility::Node *node, utility::Tree &tree, std::vector<utility::Node *> siblings)
{
    ...
    for (i = 0; i < utility::length; i++)
        up[i].resize(STATES, 1); // Initializing with 1;
...
void marginal_preorder_traversal(utility::Node *node, utility::Tree &tree)
{
    ...
    for (i = 0; i < utility::length; i++)
        marginal[i].resize(STATES, 1); // Initializing with 1;
    node->inference.resize(utility::length);
    for (i = 0; i < utility::length; i++)
    {
        float total_prob = 0;
        for (j = 0; j < STATES; j++)
            total_prob += utility::pi[j] * up[i][j] * bottom[i][j];
        for (j = 0; j < STATES; j++)
            marginal[i][j] = (utility::pi[j] * up[i][j] * bottom[i][j]) / total_prob;
        // infer characters
        float max = 0;
        for (j = 0; j < STATES; j++)
        {
        ...
        {
            if (marginal[i][j] == max)
                node->inference[i].push_back(int8_t(pow(2, j)));
        }
    }
